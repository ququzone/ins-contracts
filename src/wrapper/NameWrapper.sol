...
import {IBaseRegistrar} from "../iotxregistrar/IBaseRegistrar.sol";
    ...
    ReverseClaimer
{
    ...
    function setUpgradeContract(INameWrapperUpgrade _upgradeAddress) public onlyOwner {
        if (address(upgradeContract) != address(0)) {
            ...
            ins.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            ins.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    function renew(uint256 tokenId, uint256 duration) external onlyController returns (uint256 expires) {
        ...
        try registrar.ownerOf(tokenId) returns (address registrarOwner) {
            if (registrarOwner != address(this) || ins.owner(node) != address(this)) {
     ...
     * @param _resolver Resolver contract
     */

    function wrap(bytes calldata _name, address _wrappedOwner, address _resolver) public {
        ...
        if (owner != msg.sender && !ins.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }

        if (_resolver != address(0)) {
            ins.setResolver(node, _resolver);
        }

        ins.setOwner(node, address(this));

        _wrap(node, _name, _wrappedOwner, 0, 0);
    }
    ...
    function upgrade(bytes calldata _name, bytes calldata _extraData) public {
        ...
        upgradeContract.wrapFromUpgrade(_name, currentOwner, fuses, expiry, approved, _extraData);
    }
    ...
    function setChildFuses(bytes32 parentNode, bytes32 labelhash, uint32 fuses, uint64 expiry) public {
        ...
        if (owner == address(0) || ins.owner(node) != address(this)) {
            revert NameIsNotWrapped();
        }
        // max expiry is set to the expiry of the parent
    ...
    function setSubnodeOwner(
        ...
        if (!_isWrapped(node)) {
            ins.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    function setSubnodeRecord(
        ...
        expiry = _checkParentFusesAndExpiry(parentNode, node, fuses, expiry);
        if (!_isWrapped(node)) {
            ins.setSubnodeRecord(parentNode, labelhash, address(this), resolver, ttl);
    ...
    /**
     * @notice Sets records for the name in the INS Registry
    ...
    function setRecord(
        ...
        ins.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
    ...
    function _addLabel(string memory _label, bytes memory _name) internal pure returns (bytes memory ret) {
        if (bytes(_label).length < 1) {
            revert LabelTooShort();
        }
        if (bytes(_label).length > 255) {
            revert LabelTooLong(_label);
        }
        return abi.encodePacked(uint8(bytes(_label).length), _label, _name);
    }
    ...
    function _wrap(bytes32 _node, bytes memory _name, address _wrappedOwner, uint32 _fuses, uint64 _expiry) internal {
        _mint(_node, _wrappedOwner, _fuses, _expiry);
        emit NameWrapped(_node, _name, _wrappedOwner, _fuses, _expiry);
    }
    ...
    function _isWrapped(bytes32 node) internal view returns (bool) {
        return ownerOf(uint256(node)) != address(0) && ins.owner(node) == address(this);
    }
